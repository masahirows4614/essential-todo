...
export default function SignInForm({ onClose }: { onClose?: () => void } = {}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (!res.ok) { setError(res.error ?? "サインインに失敗しました"); return; }
    if (onClose) onClose();
    else router.push("/");
  }
    ...
    >
      ...
      <div className="mb-8 flex flex-col items-center gap-2">
        ...
        <div className="text-center">
          ...
          <span className="text-xs text-slate-400">本当に大切なことに、集中する</span>
        </div>
      </div>

      {/* Card */}
      <div className="glass-float rounded-3xl p-6 shadow-xl">
        <h2 className="mb-5 text-base font-bold text-slate-800">おかえりなさい 👋</h2>
        ...
        <p className="mt-4 text-center text-xs text-slate-400">
          アカウントがない方は{" "}
          <Link href="/signup" className="font-semibold text-indigo-500 hover:underline">
            新規登録
          </Link>
        </p>
      </div>
